...
KUBE_VERSION=$(kubectl version | grep "Server Version" | awk -F '.' '{print $2}')
...
# KServe servingruntimes service health is not monitored. Adding this retry to address this issue
...
# Loop until either the command succeeds or maximum retries are reached
while [ $retry_count -lt $max_retries ]; do
    # Check the exit status of the command
    if kubectl apply -f https://github.com/kserve/kserve/releases/download/${KSERVE_VERSION}/kserve-runtimes.yaml; then
